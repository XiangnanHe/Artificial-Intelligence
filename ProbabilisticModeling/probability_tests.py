...
class ProbabilityTests(unittest.TestCase):
    ...
    #Part 1c
    def test_probability_results(self):
        bayes_net = make_power_plant_net()
        set_probability(bayes_net)
        alarm_prob = get_alarm_prob(bayes_net)
        gauge_prob = get_gauge_prob(bayes_net)
        temp_prob = get_temperature_prob(bayes_net)
        print(alarm_prob, gauge_prob, temp_prob)

    def test_probability_results_final(self):
        bayes_net = make_final_exam_net()
        set_probability_final_exam(bayes_net)
        P_A_E_prob = get_A_given_E(bayes_net)
        P_not_G_not_N = get_not_G_given_not_N(bayes_net)
        P_C_G_E = get_C_given_G_E(bayes_net)
        P_E_G_N = get_E_given_G_N(bayes_net)
        #gauge_prob = get_gauge_prob(bayes_net)
        #temp_prob = get_temperature_prob(bayes_net)
        print P_A_E_prob, P_not_G_not_N, P_C_G_E, P_E_G_N
    ...
    def test_Gibbs_sampler(self):
        ...
        initial_state.extend([0, 0, 0, 0, 0, 2])
        Gibbs_sampler(bayes_net, initial_state)


    def test_compare_sampling(self):
        initial_state = []
        bayes_net = get_game_network()
        Gibbs_convergence, MH_convergence, Gibbs_count, MH_count, MH_rejection_count = \
            compare_sampling(bayes_net, initial_state)
